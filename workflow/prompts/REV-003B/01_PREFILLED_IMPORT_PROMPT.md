...
> Repository-staged version. Before following the tasks below, reconstruct the
> intake package from the multipart source stored on this branch:
>
> ```bash
> bash imports/staging/REV-003B/restore_package.sh
> ```
>
> The script verifies SHA-256
> `7fcbd199bd251b2aec7b4856438bc40b2f625d1c0f52fa74000ff44e59042b63`,
> extracts the package under `/tmp/PCC_REV003B_Fulltext_Intake_with_Piwek_Proxy/`,
> and prints the three exact PDF paths. Use those extracted PDFs as the source
> files for the canonical repository paths named below.
...
Use **Code mode** and base the task on `main`.
...
Copy these exact files into the repository at these paths:
...
pages: 8
sha256: 324b61fb388af4db1de1449fda5308df55d069de4ec1548c4e28370264621a44
size_bytes: 1049305
...
pages: 12
sha256: 66ddafe54cae7a4f1e13e72ba77068eeb6ebc09cbd5e191a6752fbd226ae7968
size_bytes: 1017348
...
author: Paul Piwek
year: 2000
version: ITRI-00-14 technical report; also published in LDV Forum
pages: 16
sha256: b9c15c9b63318ac82efcf5c30ff89f4e994ab09d13f8676d5ade9c8d6a786ff2
size_bytes: 187110
```

Verify all titles from page 1 and compute the checksums locally. Stop if any
title, checksum, file size, or page count differs.
...
- transfer mechanisms from the 1997 citation to the 2000 report.
...
1. Add import-manifest rows for the three exact PDFs using the next unused
   `IMP-####` IDs.
2. Preserve the original filenames, canonical stored paths, SHA-256 values,
   sizes, and `application/pdf` media type.
...
6. Add the Piwek 2000 report as a new source row using the next unused source
...
9. Create:
   ...
   listing exact versions, paths, checksums, readiness, and the outstanding
   `SRC-0004` gap.
...
Report exact files changed, source/import IDs assigned, checksums, status
changes, validation results, and the remaining `SRC-0004` access gap.
